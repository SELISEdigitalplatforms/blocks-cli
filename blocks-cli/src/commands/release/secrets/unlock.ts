...
export async function releaseSecretsUnlock(argv: string[]): Promise<void> {
  ...
  if (dryRun) {
    writeOutput({ action: "unlock", dryRun: true, repoId }, flags);
    return;
  }

  await confirmMutation(flags, `Unlock the secret set of repo '${repo.repoName ?? repoId}'.`);
  await blocksRequest<unknown>(`${RELEASE_API}/RepoSecret/unlock`, {
    body: { repoId },
    impersonatedProjectAuth: true,
    projectTenantId: projectKey,
    ...requestContext(flags)
  });
  writeOutput({ repoId, unlocked: true }, flags);
}
